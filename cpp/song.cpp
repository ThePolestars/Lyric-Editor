...
void Song::mp3Open(const char *ch)
{
    ...
    if(id3v2tag){
        ...
        if(!l.isEmpty()){
            ...
            FILE *file = NULL;
            file = fopen("../resource/image/pic.png", "wt+");
            qDebug()<<file;
            fwrite(p->picture().data(), size, 1, file);
            fclose(file);
            m_flag = true;
        }else{
            m_flag = false;
...
void Song::flacOpen(const char *ch)
{
    ...
    if(flacFile->pictureList()[0]){     //判断flac文件中是否存在图片
        FILE *file = NULL;
        file = fopen("../resource/image/pic.png", "wt+");
        fwrite(flacFile->pictureList()[0]->data().data(), flacFile->pictureList()[0]->data().size(), 1, file);
        fclose(file);
        m_flag = true;
    }else{
        m_flag = false;
...
void Song::oggOpen(const char *ch)
{
    ...
    if(vorbis){
        if(vorbis->pictureList()[0]){
            FILE *file = NULL;
            file = fopen("../resource/image/pic.png", "wt+");
            fwrite(vorbis->pictureList()[0]->data().data(), vorbis->pictureList()[0]->data().size(), 1, file);
            fclose(file);
            m_flag = true;
        }else{
            m_flag = false;
